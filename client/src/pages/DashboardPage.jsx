...
export default function DashboardPage() {
	...
	const fetchFoodForToday = async () => {
		...
		const res = await fetch(`${import.meta.env.VITE_BASE_URL}api/foods/${currentDate}`); // Assuming the backend endpoint is /api/foods
	...
	const submitFood = async food => {
		...
		if (foodCardIndex == -1) {
			...
			const res = await fetch(`${import.meta.env.VITE_BASE_URL}api/foods/${food._id}`, {
				method: 'PATCH',
				body: JSON.stringify(food),
				headers: {
					'Content-Type': 'application/json',
